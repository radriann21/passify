...
export const PasswordInput = () => {
  const { password, strength, setPassword } = usePasswordStoreContext((state) => state);
  ...
  return (
    ...
    >
      ...
      <Box>
        ...
        <Text fontSize=".8rem">Manage the options for creating the password</Text>
      </Box>
      <Flex
        w="full"
        padding="1rem"
      ...
      >
        <Box>
          <Text fontSize={{ base: ".7rem", md: ".9rem" }}>
            {password === "" ? "Your password is gonna appear here." : password}
          </Text>
        </Box>
      ...
      <Box>
        <Heading as="h2" fontSize=".9rem" mb=".5rem">
          Options
        </Heading>
        <Flex
          w="full"
        ...
        >
          <VStack spaceY=".5rem">
            <Checkbox
              colorPalette="blue"
              checked={uppercase}
              onCheckedChange={(evt) => setUppercase(!!evt.checked)}
            >
              Uppercase (A-Z)
            </Checkbox>
            <Checkbox
              colorPalette="blue"
              checked={lowercase}
              onCheckedChange={(evt) => setLowercase(!!evt.checked)}
            >
              Lowercase (a-z)
            </Checkbox>
          </VStack>
          <VStack spaceY=".5rem" alignItems="flex-start">
            <Checkbox
              colorPalette="blue"
              checked={numbers}
              onCheckedChange={(evt) => setNumbers(!!evt.checked)}
            >
              Numbers (0-9)
            </Checkbox>
            <Checkbox
              colorPalette="blue"
              checked={symbols}
              onCheckedChange={(evt) => setSymbols(!!evt.checked)}
            >
              Symbols (!@#$%^&*)
            </Checkbox>
          </VStack>
      ...
      <Flex
        ...
        bgColor={{ base: "blue.100", _dark: "gray.900" }}
      >
        <Text textTransform="uppercase" fontSize=".7rem" fontWeight="bold" letterSpacing="2px">
          Strength
        </Text>
        <Flex spaceX=".3rem" alignItems="center">
          <Text fontWeight="bold">{names[strength - 1]}</Text>
          <Flex spaceX=".3rem">
            {names.map((_, index) => (
              <Box
                key={index}
                ...
                w="20px"
                border="1px solid"
                borderColor={strength >= index + 1 ? "blue.500" : "gray.300"}
                bgColor={strength >= index + 1 ? "blue.500" : "transparent"}
